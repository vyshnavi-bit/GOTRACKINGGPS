...
public class VehicleDBMgr
{
    ...
    public  void InitializeDB()
    {
        lock (obj_lock)
        {
            ...
            //server=128.199.183.206;User Id=vehicletracking;Persist Security Info=True;database=vehicle_tracking
        }
    }
    ...
    //                {
    //                    formFields += fields[i] + ",";
    //                    param = new MySqlParameter(fields[i], Values[i]);
    //                    cmd.Parameters.Add(param);
    //                }
    //                formFields = formFields.Substring(0, formFields.LastIndexOf(","));
    //                cmd.CommandText = "insert into " + TableName + " values (" + formFields + ")";
    //                cmd.Connection = conn;
    //                conn.Open();
    //                cmd.ExecuteNonQuery();
    //                conn.Close();
